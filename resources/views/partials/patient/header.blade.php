    ...
    <div x-data="{ open: false }"
        ...
        <div class="p-4 flex flex-row items-center justify-between">
            <a href="{{ route("dashboard") }}"
                class="text-lg font-semibold tracking-widest uppercase rounded-lg focus:outline-none focus:shadow-outline">Tableau
                de bord</a>
        ...
        <nav :class="{ 'flex': open, 'hidden': !open }"
            ...
            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                <button @click="open = !open"
                    ...
                    <span>{{ Illuminate\Support\Facades\Auth::user()->name }} {{ Illuminate\Support\Facades\Auth::user()->prenom }}</span>

                    @if (Illuminate\Support\Facades\Auth::user()->patient->img_url != null)
                        <img class="inline h-6 rounded-full"
                            src="{{ asset('assets/uploads/patient/' . Illuminate\Support\Facades\Auth::user()->patient->img_url) }}">
                    @else
                        <img class="inline h-6 rounded-full" src="{{ asset('assets/images/avatar/4.jpg') }}"
                            alt="avatar">
                    @endif
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="py-2 bg-white text-blue-800 text-sm rounded-sm border border-main-color shadow-sm">
                        <a class="block px-4 py-2 mt-2 text-sm bg-white md:mt-0 focus:text-gray-900 hover:bg-indigo-100 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                            href="{{ route('patient.setting') }}">Paramètre</a>
                        <a class="block px-4 py-2 mt-2 text-sm bg-white md:mt-0 focus:text-gray-900 hover:bg-indigo-100 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                            href="{{ route('patient.consultations') }}">Consultations</a>
                            <a class="block px-4 py-2 mt-2 text-sm bg-white md:mt-0 focus:text-gray-900 hover:bg-indigo-100 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                            href="{{ route('patient.declarations') }}">Déclarations</a>
                             <a class="block px-4 py-2 mt-2 text-sm bg-white md:mt-0 focus:text-gray-900 hover:bg-indigo-100 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                             href="{{ route('patient.rendez.vous') }}">Rendez-vous</a>
