@extends('layouts.dashboard', ['title' => 'Information sur la consultation'])
@section('content')
<div class="box">
    <div class="content-header mb-20">
        <div class="d-flex align-items-center">
            <div class="me-auto">
                <h4 class="page-title">Détails <span class="fw-bold">{{ $consultation->patient->code_patient }}</span></h4>
                <div class="d-inline-block align-items-center">
                    <nav>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#"><i class="mdi mdi-home-outline"></i></a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{ $consultation->patient->user->name }} {{ $consultation->patient->user->prenom }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="box">
                <div class="profile-image mb-10" style="text-align: center">
                    @if($consultation->patient->gender == 'masculin')
                        <img src="{{ asset('assets/images/avatar/6.png') }}" class="box-shadowed rounded-circle" alt="Photo de profil"/>
                    @elseif($consultation->patient->gender == 'feminin')
                        <img src="{{ asset("assets/images/avatar/2.png")}}" class="box-shadowed rounded-circle" alt="Photo de profil"/>
                    @else
                        <img src="{{ asset('assets/uploads/patient/$item->img_url')}}" class="box-shadowed rounded-circle" alt="Photo de profil"/>
                    @endif
                </div>
                <div class="box-body">
                    ...
                    <div class="row text-center mt-10">
                        <div class="col-md-12"><strong>Residence actuelle</strong>
                            ...
                            <br> {{ $consultation->patient->habitualResidence->name }}</p>
                        </div>
                    </div>
                    <hr>
                    <div class="row text-center mt-10">
                    ...
                    <div class="row text-center mt-10">
                        <div class="col-md-12"><strong>Pièce d'identité</strong>
                            <p style="text-transform: uppercase"><span class="fw-400">{{ $consultation->patient->type_piece }}</span>
                            <br> {{ $consultation->patient->numero_identite }}</p>
                        </div>
                    </div>
                    <hr>
                    <br>
                ...
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-md-3 col-xs-6 border-end"> <strong>Type de visite</strong>
                            <br>
                            <p class="text-muted">{{ $consultation->prestationHospital->prestationService->libelle }}</p>
                        </div>
                    ...
                    <h4 class="box-title fw-500 py-20 border-bottom d-block">Ordonnance prescrite @if(App\Models\Ordonnance::where('consultation_id', $consultation->id)->exists()): <span class="fw-bold">{{ App\Models\Ordonnance::where('consultation_id', $consultation->id)->first()->reference }}</span>@endif</h4>
                    ...
                    <h4 class="box-title my-20 fw-500 py-20 border-bottom d-block">Examen(s) prescrit(s) @if(App\Models\BulletinExamen::where('consultation_id', $consultation->id)->exists()): <span class="fw-bold">{{ App\Models\BulletinExamen::where('consultation_id', $consultation->id)->first()->code_bulletin }}</span>@endif</h4>
                    ...
                    @if(App\Models\ArretTravail::where('consultation_id', $consultation->id)->exists())
                        @foreach(App\Models\ArretTravail::where('consultation_id', $consultation->id)->get() as $item)
                            <div class="d-flex no-block text-success">
                                ...
                                Date début: <span class="ms-10 text-dark badge badge-primary">{{ $item->date_debut }}</span>&nbsp;&nbsp;
                                Date fin: <span class="ms-10 text-dark badge badge-danger">{{ $item->date_fin }}</span>&nbsp;&nbsp;
                                Nombre de jour: <span class="ms-10 text-dark badge badge-info">{{ $item->nb_jour }}</span>&nbsp;&nbsp;
                            </div>
                        @endforeach
                    @else
                    <div class="d-flex no-block fa fa-check-circle text-danger">
